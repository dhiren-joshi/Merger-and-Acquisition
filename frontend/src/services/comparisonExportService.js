...
export const exportComparisonToPDF = async (deals) => {
    try {
        ...
        const metrics = [
            ...
            { label: 'Industry Match', getValue: (d) => `${Math.round(d.fitScore?.metricBreakdown?.industryMatch?.weightedScore || 0)}` },
            { label: 'Financial Health', getValue: (d) => `${Math.round(d.fitScore?.metricBreakdown?.financialHealth?.weightedScore || 0)}` },
            { label: 'Cultural Fit', getValue: (d) => `${Math.round(d.fitScore?.metricBreakdown?.culturalFit?.weightedScore || 0)}` },
            { label: 'Tech Compatibility', getValue: (d) => `${Math.round(d.fitScore?.metricBreakdown?.technologyCompatibility?.weightedScore || 0)}` },
            { label: 'Strategic Alignment', getValue: (d) => `${Math.round(d.fitScore?.metricBreakdown?.strategicAlignment?.weightedScore || 0)}` },
